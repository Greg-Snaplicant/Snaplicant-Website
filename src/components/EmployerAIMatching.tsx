          ...
          <div className="bg-gradient-to-r from-green-500/20 to-blue-500/20 rounded-2xl p-6 border border-green-500/30 mb-8">
            ...
            <div className="flex items-center space-x-6 text-sm">
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className="text-white/80">Average match score: {matches.length > 0 ? Math.round(matches.reduce((acc, m) => acc + m.matchScore, 0) / matches.length) : 0}%</span>
              </div>
            </div>
          </div>
        ) : (
          <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-12 border border-white/10 mb-8 text-center">
            <Users className="w-16 h-16 text-white/40 mx-auto mb-6" />
            <h2 className="text-2xl font-bold text-white mb-4">No Matching Candidates Found</h2>
            <p className="text-white/80 mb-6">
              <strong>Your search:</strong> "{searchQuery}"
            </p>
            <p className="text-white/60 mb-8">
              We couldn't find any candidates matching your criteria. This could be because:
            </p>
            <ul className="text-white/60 text-left max-w-md mx-auto mb-8 space-y-2">
              <li>• No candidates have joined the platform yet</li>
              <li>• Your search criteria might be too specific</li>
              <li>• Matching candidates haven't created profiles yet</li>
            </ul>
            <button
              onClick={handleNewSearch}
